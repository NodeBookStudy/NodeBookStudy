...
const app = express();
let posts = []; // 1. 게시글 리스트에 사용할 posts에 빈 리스트 할당

// req.body를 사용할려면 JSON 미들웨어를 사용해야 합니다.
...
app.delete("/posts/:id", (req, res) => {
  ...
  const filteredPosts = posts.filter((post) => post.id !== +id); // 10. 글 삭제 로직(+id : 문자열인 id를 숫자형으로 변경[=parseInt()])
